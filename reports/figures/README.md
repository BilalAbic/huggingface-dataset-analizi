...
The ten figures in this directory are generated from the fixed JSON evidence in
...
| [`topic-overlap.png`](topic-overlap.png) | Which datasets cover the same ground? | Chord diagram | `outputs/topic_overlap.json` | Edges are TF-IDF vocabulary cosine; solid edges reach the 0.30 redundancy threshold, dashed ones are shown from 0.15 for context. Node size is topic concentration. **Shared vocabulary is not shared meaning** — the two English datasets score high on function words alone, so read a pair before calling it redundant. |
...
The script uses Matplotlib and writes all ten PNG files deterministically from
